...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    @Autowired
    private IStaffLocalRecordService staffLocalRecordService;
    ...
    @Override
    public void registDelivery(String username, String password, String name) throws Exception {
        User user = new User();
        user.setPassword(MD5Util.encrypt(username, password));
        user.setUsername(username);
        user.setCreateTime(new Date());
        ...
        staffInfo.setCanteenId(1);
        staffInfoService.save(staffInfo);

        StaffLocalRecord staffLocalRecord = new StaffLocalRecord();
        staffLocalRecord.setStaffId(staffInfo.getId());
        staffLocalRecord.setLng(BigDecimal.valueOf(116.41724700));
        staffLocalRecord.setLat((BigDecimal.valueOf(39.91835100)));
        staffLocalRecord.setCreateDate(DateUtil.formatDateTime(new Date()));
        staffLocalRecordService.save(staffLocalRecord);
    ...
    public void registStaff(String username, String password, StaffInfo staffInfo) throws Exception {
        ...
        StaffLocalRecord staffLocalRecord = new StaffLocalRecord();
        staffLocalRecord.setStaffId(staffInfo.getId());
        MerchantInfo merchantInfo = merchantInfoService.getById(staffInfo.getMerchantId());
        if (merchantInfo != null) {
            staffLocalRecord.setLng(merchantInfo.getLongitude());
            staffLocalRecord.setLat(merchantInfo.getLatitude());
            staffLocalRecord.setCreateDate(DateUtil.formatDateTime(new Date()));
            staffLocalRecordService.save(staffLocalRecord);
        }
